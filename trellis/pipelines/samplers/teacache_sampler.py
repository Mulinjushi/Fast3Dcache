...
class TeaCacheSampler(FlowEulerSampler):
    """
    TeaCache sampler。
    """
    def __init__(
        ...
        print(f"TeaCacheSampler initialized with cache_threshold (δ) = {self.cache_threshold}")
        ...
        **kwargs
        ...
        for t, t_prev in t_iter:
            ...
            if last_proxy_t_emb is None:
                ...
                if proxy_accumulator <= self.cache_threshold:
                    is_cache_step = False
                else:
                    is_cache_step = True
                    proxy_accumulator = 0.0
            ...
            if is_cache_step:
                t_iter.set_description(f"TeaCache: Computing at t={t:.4f}")
                ...
                t_iter.set_description(f"TeaCache: Reusing cache at t={t:.4f}")
                if cached_output_v is None:
                    raise RuntimeError("TeaCache logic error: Attempted to reuse cache, but cache is empty.")
