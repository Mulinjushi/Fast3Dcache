# trellis/pipelines/trellis_image_to_3d_teaf3c.py
from typing import *
...
class TrellisImageTo3DPipeline_yesfinal(Pipeline):
    ...
    @staticmethod
    def from_pretrained(path: str) -> "TrellisImageTo3DPipeline_yesfinal":
        pipeline = super(TrellisImageTo3DPipeline_yesfinal, TrellisImageTo3DPipeline_yesfinal).from_pretrained(path)
        new_pipeline = TrellisImageTo3DPipeline_yesfinal()
    ...
    def sample_sparse_structure(
        ...
        teacache_params: dict = {},
        
        use_f3c_teacache_nested: bool = False,
        f3c_teacache_nested_params: dict = {}
        ...
        sampler_params_copy = {**self.sparse_structure_sampler_params, **sampler_params}
        sampler = None
        
        if use_f3c_teacache_nested:
            from trellis.pipelines.samplers.f3c_teacache_nested_sampler import F3CTeaCacheNestedSampler
            sampler = F3CTeaCacheNestedSampler(
                sigma_min=self.sparse_structure_sampler.sigma_min,
                **f3c_teacache_nested_params
            )
            print(f"Using F3C-TeaCache Nested Sampler")
        
        elif use_f3c:
            from fast3Dcache.f3c_flow_euler import F3cFlowEulerCfgSampler
            sampler = F3cFlowEulerCfgSampler(
                sigma_min=self.sparse_structure_sampler.sigma_min
            )
            print(f"Using F3C (L2 Only) Sampler")

        elif use_teacache:
            from trellis.pipelines.samplers.teacache_sampler import TeaCacheSampler
            sampler = TeaCacheSampler(
                sigma_min=self.sparse_structure_sampler.sigma_min,
                **teacache_params
            )
            print(f"Using TeaCache (L1 Only) Sampler")

        else:
            sampler = self.sparse_structure_sampler
        
        f3c_extra_args = {}
        if use_f3c or use_f3c_teacache_nested:
            if 'f3c_args' not in sampler_params_copy:
                raise ValueError("F3C samplers require 'f3c_args' to be passed in 'sparse_structure_sampler_params'.")
            
            f3c_args = sampler_params_copy.pop('f3c_args')
            
            if 'effective_steps' not in f3c_args:
                f3c_args.effective_steps = sampler_params_copy.get('steps', total_steps)
            
            f3c_args.resolution = flow_model.resolution // flow_model.patch_size

            LEADER.set_parameters(f3c_args) 
    ...
    def run(
        ...
        use_f3c_for_structure: bool = False,
        
        use_teacache_for_structure: bool = False,
        teacache_params: dict = {},
        
        use_f3c_teacache_nested_for_structure: bool = False,
        f3c_teacache_nested_params: dict = {}
    ) -> dict:
        if preprocess_image:
        ...
        coords, z_s_result = self.sample_sparse_structure(
            ...
            use_f3c=use_f3c_for_structure,
            
            use_teacache=use_teacache_for_structure,
            teacache_params=teacache_params,
            
            use_f3c_teacache_nested=use_f3c_teacache_nested_for_structure,
            f3c_teacache_nested_params=f3c_teacache_nested_params
        )
    ...
    def run_multi_image(self, images: List[Image.Image], num_samples: int = 1, seed: int = 42, sparse_structure_sampler_params: dict = {}, slat_sampler_params: dict = {}, formats: List[str] = ['mesh', 'gaussian', 'radiance_field'], preprocess_image: bool = True, mode: Literal['stochastic', 'multidiffusion'] = 'stochastic') -> dict:
        ...
        with self.inject_sampler_multi_image('sparse_structure_sampler', len(images), ss_steps, mode=mode):
            coords, _ = self.sample_sparse_structure(cond, num_samples, sparse_structure_sampler_params) 
