# trellis/pipelines/trellis_image_to_3d_easyf3c.py
...
from .samplers.easy_f3c_nested_sampler import EasyF3CNestedSampler


class TrellisImageTo3DEasyF3CPipeline(Pipeline):
    ...
    @staticmethod
    def from_pretrained(path: str) -> "TrellisImageTo3DEasyF3CPipeline":
        pipeline = super(TrellisImageTo3DEasyF3CPipeline, TrellisImageTo3DEasyF3CPipeline).from_pretrained(path)
        new_pipeline = TrellisImageTo3DEasyF3CPipeline()
    ...
    def sample_sparse_structure(
        ...
        num_samples: int = 1,
        sampler_params: dict = {},
        use_easy_f3c_nested: bool = False,
        easy_f3c_nested_params: dict = {} 
        ...
        # --- L1+L2 融合采样器 ---
        if use_easy_f3c_nested:
            sampler = EasyF3CNestedSampler(
                sigma_min=self.sparse_structure_sampler.sigma_min,
                **easy_f3c_nested_params
            )
            print(f"Using EasyF3C Nested Sampler (L1+L2)")
            
            if 'f3c_args' not in sampler_params_copy:
                raise ValueError("EasyF3C sampler requires 'f3c_args' to be passed in 'sparse_structure_sampler_params'.")
            
            f3c_args = sampler_params_copy.pop('f3c_args')
            
            if 'effective_steps' not in f3c_args:
                f3c_args.effective_steps = sampler_params_copy.get('steps', 25)
            ...
            print(f"Using Default Sampler ({type(sampler).__name__})")
    ...
    def run(
        ...
        use_easy_f3c_nested_for_structure: bool = False,
        easy_f3c_nested_params: dict = {} 
        ...
        coords, z_s_result = self.sample_sparse_structure(
            ...
            use_easy_f3c_nested=use_easy_f3c_nested_for_structure,
